...
test_input1 = '''acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf'''
...
input = '''badf gadfec bgcad ad dbcfg gcaeb fecdgab gad bgcadf efcdgb | gcadb ad agd deacfg
...
lines = input.split('\n')
lines = [tuple(tuple(''.join(sorted(x)) for x in y.split(' ')) for y in l.split(' | ')) for l in lines]
...
def get_choice(pool, subset = None):
    subset = subset or pool
    pick = random.choice([c for c in subset if c in pool])
    pool.remove(pick)
    return pick
...
for l in lines:
    ...
    while True:
        mapping = {}
        pool = [c for c in 'abcdefg']

        mapping['c'] = get_choice(pool, one)
        mapping['f'] = get_choice(pool, one)
        mapping['a'] = get_choice(pool, seven)
        mapping['b'] = get_choice(pool, four)
        mapping['d'] = get_choice(pool, four)

        for c in 'eg':
            mapping[c] = get_choice(pool)

        mapped = [''.join(sorted(mapping[c] for c in num)) for num in nums]
    
        if all(m in l[0] for m in mapped): break
